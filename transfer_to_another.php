...
.sidebar{width:300px;background:#e2688a;height:100vh;position:fixed;padding:30px 0;}
...
.header{margin:40px;text-align:center;color:#d85375;}
.transfer{margin:40px auto;width:60%;}
.transfer h3{color:#e2688a;}
input, select{width:100%;padding:12px;margin-top:8px;margin-bottom:20px;border-radius:5px;border:1px solid #ccc;}
.transfer_amount{background:#e2688a;padding:20px;border-radius:5px;}
.transfer_amount h3{color:#fff;}
.transfer_button{text-align:center;}
button{margin-top: 20px; padding:10px 20px; background:#d85375; border:none; color:white; font-size:16px; border-radius:5px; cursor:pointer;}
button:hover{opacity:.9;}

/* =======================
   CUSTOM POPUP
======================= */
.popup {display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.3); z-index: 9999; justify-content: center; align-items: center;}
.popup-content {background: pink; padding: 30px; border-radius: 10px; text-align: center; width: 350px; box-shadow: 0 5px 15px rgba(0,0,0,0.3);}
.popup-content p {margin-bottom: 20px; font-size: 16px; color: #333;}
.popup-buttons button {margin: 0 10px; padding: 8px 20px; border: none; border-radius: 5px; background: #d85375; color: #fff; cursor: pointer;}
...
function showPopup(message, type = "alert", callback) {
  ...
  if(type === "alert"){ // Only OK button
    btnOk.style.display = "inline-block";
    btnCancel.style.display = "none";
  } else if(type === "confirm"){ // OK + Cancel buttons
    btnOk.style.display = "inline-block";
...
function sendTransfer(e){
  ...
  if(!amount || !bank || !account){
    showPopup("Please complete all fields.", "alert");
    return;
  }
  ...
  showPopup(
    ...
    function(confirmed) {
      if(!confirmed){
        showPopup("Transfer cancelled.", "alert");
        return;
      }
      ...
      xhr.onload = () => {
        ...
        showPopup(res.message, "alert", function(){
      ...
      xhr.onerror = () => showPopup("Network error.", "alert");
